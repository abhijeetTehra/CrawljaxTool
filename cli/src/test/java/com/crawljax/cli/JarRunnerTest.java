...
public class JarRunnerTest {
    ...
    public void whenRemoteBrowserSpecifiedWithUrlItResumes() {
        JarRunner runner = new JarRunner(defaultArgsPlus(
                "-b " + BrowserType.REMOTE.name() + " -" + ParameterInterpeter.BROWSER_REMOTE_URL + " localhost:9000"));
        assertThat(streams.getErrorOutput(), isEmptyString());
    ...
    public void testWithMaxCrawlTime() {
        assertThat(configForArgs("-t 123").getMaximumRuntime(), is(TimeUnit.MINUTES.toMillis(123)));
    }

    @Test
    public void testWaitAfterReload() {
        CrawlRules crawlRules = configForArgs("-" + ParameterInterpeter.WAIT_AFTER_RELOAD + " 123")
                .getCrawlRules();
