...
class ParameterInterpeter {
    ...
    private Options getOptions() {
        ...
        options.addOption("b", BROWSER, true, "browser type: " + availableBrowsers() + ". Default is Firefox");
        ...
        options.addOption(
                "a", CRAWL_HIDDEN_ANCHORS, false, "Crawl anchors even if they are not visible in the browser.");

        options.addOption("t", TIME_OUT, true, "Specify the maximum crawl time in minutes");

        options.addOption(
                CLICK, true, "a comma separated list of HTML tags that should be clicked. Default is A and BUTTON");
    ...
    private void whenRemoteBrowserNeedsUrl() {
        if (specifiesBrowser() && getSpecifiedBrowser() == BrowserType.REMOTE) {
            checkArgument(
                    !isNullOrEmpty(getSpecifiedRemoteBrowser()), "When using remote browser the URL cannot be null");
        }
    }
    ...
    BrowserType getSpecifiedBrowser() {
        ...
        throw new IllegalArgumentException(
                "Unrecognized browser: '" + browser + "'. Available browsers are: " + availableBrowsers());
    }
    ...
    void printHelp() {
        ...
        helpFormatter.printHelp(
                writer, ROW_WIDTH, HELP_MESSAGE, "", options, SPACES_AFTER_OPTION, SPACES_BEFORE_OPTION, "");
        writer.flush();
    }
