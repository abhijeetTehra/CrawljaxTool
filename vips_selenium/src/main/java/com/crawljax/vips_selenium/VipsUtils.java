...
public class VipsUtils {
    ...
    static String COMPUTEDSTYLESHEET_ALL =
            "return Array.from(%s).map(element => {return {xpath: element, attributes: getVipsAttributes(element)}});";
    static String CDP_COMPUTEDSTYLESHEET_ALL =
            "Array.from(%s).map(element => {return {xpath: element, attributes: getVipsAttributes(element)}});";
    ...
    public static ArrayList<Node> getChildren(Node vipsBlock) {
        if (vipsBlock == null
                || vipsBlock.getNodeName().equalsIgnoreCase("#text")
    ...
    //	static String COMPUTEDSTYLESHEET_ALL = "return Array.from(document.querySelectorAll('*')).map(element => {return
    // {webElement: element, computedStyle: getComputedStyle(element)}});";
    ...
    public static Rectangle peelRectangleLayer(Rectangle rect, String toRemove) {
        ...
        Rectangle returnRect =
                new Rectangle(rect.x + left, rect.y + top, rect.width - (left + right), rect.height - (top + bottom));
    ...
    public static Rectangle getContentRectangle(Node node, WebDriver driver) {
        ...
        String javascriptReturn = ((JavascriptExecutor) driver)
                .executeScript(CONTENT_RECTANGLE_JAVASCRIPT_FUNCTION + CONTENT_RECTANGLE_RETURN, vipsBlock)
                .toString();
        ...
        Rectangle outer = new Rectangle(outer_return.x, outer_return.y, outer_return.width, outer_return.height);
    ...
    public static Rectangle getRectangle(Node vipsBlock, WebDriver driver) {
        ...
        if (vipsBlock.getAttributes().getNamedItem(RECTANGLE) != null) {
            Gson gson = new Gson();
            String rectString =
                    vipsBlock.getAttributes().getNamedItem(RECTANGLE).getNodeValue();
    ...
    private static Rectangle getInBetweenRectangle(Rectangle beforeRect, Rectangle afterRect, Rectangle rect) {
    ...
    public static int getFontSize(Node vipsBlock, WebDriver driver) {
        ...
        if (vipsBlock.getAttributes().getNamedItem(FONTSIZE) != null) {
            ...
            String propertyValue =
                    vipsBlock.getAttributes().getNamedItem(FONTSIZE).getNodeValue();
    ...
    private static String getCssProperty(Node vipsBlock, String property, WebDriver driver) {
        ...
        if (vipsBlock.getAttributes().getNamedItem(property.toLowerCase()) != null) {
            ...
            String propertyValue = vipsBlock
                    .getAttributes()
                    .getNamedItem(property.toLowerCase())
                    .getNodeValue();
    ...
    public static void populateStyle(Document dom, WebDriver driver, boolean USE_CDP) {
        ...
        //		Gson gson = new Gson();
        //		List<Map<String, VipsBrowserAttributes>> attributes = gson.fromJson( attributeString, new
        // TypeToken<List<Map<String, VipsBrowserAttributes>>>(){}.getType());
            ...
            for (Object elementSheet : (Collection) attributeString) {
                if (elementSheet instanceof Map) {
                    ...
                    if (attributes instanceof Map) {
                        ...
                        attributeMap.put(
                                xpath,
                                new VipsBrowserAttributes(
                                        rect, fontSize, fontWeight, bgColor, isDisplayed, eventListeners));
                    }
                }
    ...
    private static void setBrowserAttributes(Node vipsBlock, VipsBrowserAttributes browserAttributes) {
        ...
        boolean isDisplayed = browserAttributes.isDisplayed()
                && browserAttributes.rectangle != null
                && browserAttributes.rectangle.height > 0
                && browserAttributes.rectangle.width > 0;
        setIsDisplayed(vipsBlock, isDisplayed ? "true" : "false");
    ...
    public static int getFragParent(Node vipsBlock) {
        ...
        if (vipsBlock.getAttributes().getNamedItem(FRAGPARENT) != null) {
            String propertyValue =
                    vipsBlock.getAttributes().getNamedItem(FRAGPARENT).getNodeValue();
            return Integer.parseInt(propertyValue);
        }
