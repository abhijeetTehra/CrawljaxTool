...
public final class VipsOutput {
    ...
    private void writeVisualBlocks(Element parentNode, VisualStructure visualStructure, int level) {
        ...
        layoutNode.setAttribute(
                "SourceIndex",
                XPathHelper.getXPathExpression(visualStructure.getNestedBlocks().get(0)));
        ...
        layoutNode.setAttribute(
                "DOMCldNum", String.valueOf(parentBox.getChildNodes().getLength()));
    ...
    public int drawVisualStructure(
        ...
        Rectangle rect2 = new Rectangle(
                visualStructure.getX(),
                visualStructure.getY(),
        ...
        if (!rect2.contains(rect1)
                ...
                && rect1.width * rect1.height > 0
                && rect2.width * rect2.height > 0) {
        ...
        VipsRectangle me = new VipsRectangle(
                visualStructure.getNestedBlocks(), id, parentId, XPathHelper.getXPathExpression(vipsBlock), finalRect);
    ...
    public List<VipsRectangle> exportVisualStructureToImage(
            VisualStructure visualStructure,
            BufferedImage pageViewport,
            File target,
            boolean fragOutput,
            WebDriver driver) {
        LOG.debug(visualStructure.getNestedBlocks().toString());
        BufferedImage image =
                new BufferedImage(pageViewport.getWidth(), pageViewport.getHeight(), pageViewport.getType());
    ...
    public void writeXML(VisualStructure visualStructure, BufferedImage pageViewport, String url, String title) {
        try {
            ...
            //			String pageTitle =
            // pageViewport.getRootElement().getOwnerDocument().getElementsByTagName("title").item(0).getTextContent();
