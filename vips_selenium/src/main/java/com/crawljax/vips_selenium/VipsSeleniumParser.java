...
public class VipsSeleniumParser {
    ...
    public void parse() {
        if (_viewport != null && this.dom != null) {
            ...
            this._vipsBlocks =
                    dom.getDocumentElement().getElementsByTagName("body").item(0);
    ...
    private boolean verifyValidity(Node node) {
        ...
        if (rect.getX() + rect.getWidth() > _pageWidth) {
            return false;
            // LOG.debug("X " + node.getAbsoluteContentX() + "\t" + (node.getAbsoluteContentX() +
            // node.getContentWidth()) + "\t" + _pageWidth);
        }

        if (rect.getY() + rect.getHeight() > _pageHeight) {
            return false;
            // LOG.debug("Y " + node.getAbsoluteContentY() + "\t" + (node.getAbsoluteContentY() +
            // node.getContentHeight()) + "\t" + _pageHeight);
        }
    ...
    private void isOnlyOneDomSubTree(Node pattern, Node node, boolean result) {
        ...
        for (int i = 0; i < VipsUtils.getChildren(pattern).size(); i++) {
            isOnlyOneDomSubTree(
                    VipsUtils.getChildren(pattern).get(i),
                    VipsUtils.getChildren(node).get(i),
                    result);
        }
    }
